...
class Environment():
    ...
    def _move(self, state, action):                     # ある状態である行動をすると、次にどの状態になるかを返す関数を定義
        ...
        if action == Action.UP:
            ...
            if self.grid[next_state.row][2] == -1: # Action.Branchで同じ処理をしている
                ...
                print("\n今の状態 {} : BRANCH TRUE".format(state))
            else:
                print("今の状態 {} : NO BRANCH !!!!!".format(state))
    ...
    def reward_func(self, state, TRIGAR):                       # 報酬関数を定義
        ...
        if TRIGAR:
            ...
            if attribute == 0:
                print("Node 不一致 (On the way go)")
                reward = self.default_reward
            else:
                print("Node 一致 (On the way go)")
                reward = 0
            # reward = reward * 1 # self.default_reward
        done = False
        ...
        # elif attribute == 1 and not TRIGAR:
        #     print("Node 一致 (On the way go)")
